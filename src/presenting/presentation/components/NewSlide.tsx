...
import { addNewSlide } from "@/presenting/state/slices/presentationGeneration";
...
} from "@/presenting/editor/importing/template-v2-import";
import {
...
const NewSlideV1 = ({
  ...
  const presentationLayout = useSelector(
    (state) => state.generation.presentationData?.layout
  );
  const slideCount = useSelector((state) => {
    const slides = state.generation.presentationData?.slides;
