...
const Chat = ({
  ...
  // llmConfig is passed as a prop via ChatProps (bridged from Cowork's model selection)
  const llmConfig = useSelector(
    (state) => (state as any).userConfig?.llm_config ?? {},
  );
  ...
  const chatHtmlSelection = useSelector(
    (state) => state.generation.chatHtmlSelection,
  );
