...
import { usePathname } from "@/presenting/compat/navigation";
import { useDispatch, useSelector, useStore } from "@/presenting/compat/redux";
...
import { addToHistory } from "@/presenting/state/slices/undoRedoSlice";
;
...
const SlideActionBar = ({
  ...
  const isStreaming = useSelector(
    (state) => state.generation.isStreaming
  );
  const slideCount = useSelector((state) => {
    const currentSlides = state.generation.presentationData?.slides;
    return Array.isArray(currentSlides) ? currentSlides.length : 0;
  });
  const hasPresentation = useSelector(
    (state) => Boolean(state.generation.presentationData)
  );
  const isTemplateFree = useSelector((state) =>
    isTemplateFreePresentation(
      state.generation.presentationData
    )
  );
  ...
  const rememberSlides = (actionType: string) => {
    const currentSlides = (store.getState() as any)
      .generation.presentationData?.slides;
