...
export function trimTextToWordLimit(value: string, maxWords = MAX_OUTLINE_CONTENT_WORDS): string {
...
export function limitOutlines<T extends { content?: unknown }>(outlines: T[] | null | undefined): { content: string }[] {
	if (!Array.isArray(outlines)) return [];
	return outlines.slice(0, MAX_NUMBER_OF_SLIDES).map((outline) => ({
		...outline,
		content: trimTextToWordLimit(typeof outline?.content === "string" ? outline.content : String(outline?.content ?? "")),
	}));
}
...
export function parseLimitedSlideCount(value: string | null | undefined): number | null {
